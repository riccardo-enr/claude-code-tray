...
| 260725-tsp | Trends panel shows tokens: the 24-column graph now plots tokens burned per clock hour (was quota % added), plus a "spent today 212.4M | wk 879.7M" text row ("spent today 209.8M | wk 876.0M") next to the existing rates. tokens_used is None on every record since the --api switch, so trend_spent integrates burn (tok/min) over sub-GAP_MAX intervals -- a floor, never extrapolated across a daemon outage. fmt_tokens gained a G tier. Python-only; the Rust panel sizes to content | 2026-07-25 | a21ad20 | Complete | [260725-tsp-show-tokens-spent-in-trends-panel](./quick/260725-tsp-show-tokens-spent-in-trends-panel/) |
