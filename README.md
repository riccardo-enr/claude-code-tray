...
sessions. Shows each running session's status (`running` / `waiting` / `done`)
and, on click, jumps to the tmux pane that fired it and raises the terminal
window.
...
- `claude-monitor.py` — long-lived helper: draws the AppIndicator tray menu,
  tracks sessions by `session_id`, focuses on click.
